...
clear all;clc;
tic
%% Folder containing all matlab files and scan data
...
i = 1;
NumOfAntennaPos = 400;%51;
for s = 1:2
    if s == 1
        %% Empty Scan folder
        scan_folder = 'Scan_Empty_Close';
...
X_start = (Settings{20,2}-115)/100;%meters
X_stop  = (Settings{21,2}-115)/100;%meters
X_Points = Settings{22,2};
Y_start = Settings{23,2}/100;%meters
Y_stop  = Settings{24,2}/100;%meters
Y_Points = Settings{25,2};
P = Settings{7,2};%Signal power
%%%%%%%%%%%%%%%%% Frequency %%%%%%%%%%%%%%%%%
...
%% Obtaining delay for grid pixles as if there were targets there
    for n = 1:N % n = antenna location
        ...
        D_temp = D(:,:,n);
        D_temp = D_temp(:)';
        Rn(:,:,n) = exp(j*w*D_temp);%P*exp(j*w*D_temp);
    end
    %% Constructing Image
    for q = 1:numel(Qx)
        I(q) = 0;
...
%     Fa = figure('units','normalized','outerposition',[0 0 1 1])
figure(1)
...
%     saveas(Fa,['C:\Users\' PC '\Dropbox\Spring18\SDII\Scan\Sum' num2str(n) '.jpg']);
%     close all
toc